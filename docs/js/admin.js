...
window.showSection = function(id) {
  ...
  else if (id === 'complaints') { try { renderComplaints(); } catch(e){} }
  ...
  else if (id === 'settings') { try { renderQuotes(); } catch(e){} }
...
window.renderMembers = async function() {
  ...
  // Use local storage functions instead of API calls
  let members = [];
  try {
    members = listMembers(); // Use the local storage function
  } catch (e) {
    console.warn("Failed to get members from local storage", e);
    members = [];
  }
...
window.openMemberModal = async (memberId = null) => {
  ...
  if (memberId) {
    ...
    const members = await window.App.store.list('members') || [];
    ...
    if (member) {
      ...
      if (member.branch_access && Array.isArray(member.branch_access)) {
        Array.from(branchSelect.options).forEach(option => {
          if (member.branch_access.includes(option.value)) {
...
window.syncMembers = async () => {
    ...
    try {
        const members = await window.App.api.list('members');
        window.App.store.set('members', members);
        renderMembers();
...
document.getElementById('member-form')?.addEventListener('submit', async (e) => {
  ...
  try {
    await window.App.api.save('members', memberData);
    window.App.store.upsert('members', memberData);
    renderMembers();
    closeMemberModal();
...
window.renderBranches = async function() {
  ...
  branches.forEach(branch => {
    ...
    const isClosed = type === 'closed' || !!(branch.closed===true);
    ...
    if (isClosed) {
        ...
        statusClass = 'bg-slate-900/30 text-blue-400 border border-blue-800/50';
        statusText = 'افتتاح';
    }
...
document.getElementById('branch-form')?.addEventListener('submit', async (e) => {
  ...
  try {
    await window.App.api.save('branches', branchData);
...
function updateViolationsKPIs(rows) {
  ...
  section.querySelector('#kpi-open').textContent = open;
  section.querySelector('#kpi-closed').textContent = closed;
  section.querySelector('#kpi-total').textContent = sum.toLocaleString();
  section.querySelector('#kpi-ratio').textContent = total > 0 ? `${Math.round((closed / total) * 100)}%` : '0%';
}
...
function renderViolationsTable(rows) {
  ...
  const tbody = section.querySelector('#violations-table-body');
  if (!tbody) return;
...
window.saveViolation = async function() {
  const id = document.getElementById('violation-id').value;
  const data = {
    type: document.getElementById('violation-type').value,
    amount: parseFloat(document.getElementById('violation-amount').value) || 0,
    date: document.getElementById('violation-date').value,
    branch: document.getElementById('violation-branch').value,
    paid: document.getElementById('violation-paid').checked,
  };

  try {
    if (id) {
      await window.App.api.update('violations', id, data);
      window.App.store.update('violations', id, data);
      if (typeof toast === 'function') toast('success', 'تم التحديث', 'تم تحديث المخالفة بنجاح');
    } else {
      const newViolation = await window.App.api.create('violations', data);
      window.App.store.add('violations', newViolation);
...
window.saveTask = function() {
  ...
  const data = {
    id: window.__taskEditingId || document.getElementById('task-id')?.value,
    ...
    branch_id: document.getElementById('task-branch')?.value || '',
  ...
  else window.closeModal('task-modal');
...
function getBranchName(branchId) {
  ...
  try {
    const branches = window.App.store.list('branches') || [];
    ...
    return branch ? (branch.name_ar || branch.name_en || branch.name || branch.title) : null;
  } catch (error) {
...
window.renderTasks = function(){
  ...
  rows.innerHTML = '';
  if(!norm.length){
    rows.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#94a3b8;padding:15px">لا توجد مهام</td></tr>';
    return;
  }
  ...
  norm.slice().reverse().forEach((t, index)=>{
    ...
    tr.innerHTML = `
      ...
      <td class="p-2 text-slate-200">${escapeAdmin(t.title)}</td>
      <td class="p-2 text-slate-300">${escapeAdmin(t.assignee || '—')}</td>
      <td class="p-2 text-slate-300">${escapeAdmin(getBranchName(t.branch_id || t.branch) || '—')}</td>
      ...
      <td class="p-2 text-center"><span class="px-2 py-0.5 rounded text-[11px] text-white ${statusClass}">${escapeAdmin(statusLabelAdmin(t.status))}</span></td>
      <td class="p-2 flex gap-2 justify-center">
        <button class="text-blue-400 hover:text-blue-300 transition-colors p-1" title="تعديل" onclick="editTask('${escapeAdmin(t.id)}')">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/></svg>
        </button>
        <button class="text-red-400 hover:text-red-300 transition-colors p-1" title="حذف" onclick="deleteTask('${escapeAdmin(t.id)}')">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/><path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/></svg>
        </button>
      </td>
    `;
...
window.editTask = function(id){
  ...
  // Set branch value if available
  const branchSelect = document.getElementById('task-branch');
  if (branchSelect) {
    // If branch options are not loaded yet, they will be loaded by openTaskModal
    // We should try to set the value after a short delay or check if loaded
    // But openTaskModal handles loading. We just need to set the value.
    // However, since loading is async, we might need to wait.
    
    // Better approach: Pass the task object or branch ID to openTaskModal or set it after modal opens
    // But openTaskModal is generic.
    
    // Let's set it immediately. If options exist, it will be selected.
    // If not, openTaskModal will load options. We need to re-select after loading.
    
    branchSelect.value = task.branch_id || (task.branch && task.branch.id) || '';
    
    // Store the branch ID to be set after options are loaded
    branchSelect.dataset.selectedId = task.branch_id || (task.branch && task.branch.id) || '';
  }
...
window.closeModal = function(id) {
  ...
  // Define openTaskModal early to ensure it's available when the button is clicked
  if (typeof window.openTaskModal === 'undefined') {
    window.openTaskModal = async function() {
      ...
      if (typeof window.forceLoadBranches === 'function') {
        await window.forceLoadBranches();
      }
    };
  }
  window.closeTaskModal = function() {
    ...
    // --- Enhanced Branch Loading Logic ---
    window.forceLoadBranches = async function() {
        console.log('Force loading branches initiated...');
        const branchSelect = document.getElementById('task-branch');
        if (!branchSelect) {
            console.error('Task branch select element not found!');
            return;
        }

        // Preserve selected value if any
        const currentVal = branchSelect.value || branchSelect.dataset.selectedId;
        branchSelect.innerHTML = '<option value="">جارِ التحميل...</option>';
        
        let branches = [];

        // 1. Try API first (Most reliable source of truth)
        try {
            if (window.App && window.App.api) {
                console.log('Attempting API load...');
                const apiResult = await window.App.api.list('branches');
                if (Array.isArray(apiResult) && apiResult.length > 0) {
                    branches = apiResult;
                    // Update cache
                    if (window.App.store) window.App.store.set('branches', branches);
                    localStorage.setItem('admin_branches_data', JSON.stringify(branches));
                    console.log('Loaded from API:', branches.length);
                }
            }
        } catch (e) {
        ...
        // 2. Try LocalStorage 'admin_branches_data' (Used by branches page)
        if (branches.length === 0) {
            try {
                const stored = localStorage.getItem('admin_branches_data');
                if (stored) {
                    const parsed = JSON.parse(stored);
                    if (Array.isArray(parsed) && parsed.length > 0) {
                        branches = parsed;
                        console.log('Loaded from admin_branches_data:', branches.length);
                    }
                }
            } catch (e) { console.warn('LocalStorage admin_branches_data failed:', e); }
        }

        // 3. Try LocalStorage 'branches' (Generic fallback)
        if (branches.length === 0) {
            try {
                ...
                if (stored) {
                    ...
                    if (Array.isArray(parsed) && parsed.length > 0) {
                        branches = parsed;
                        console.log('Loaded from branches cache:', branches.length);
                    }
                }
            } catch (e) { console.warn('LocalStorage branches failed:', e); }
        }

        // 4. Last Resort: DOM Scraping
        if (branches.length === 0) {
            const rows = document.querySelectorAll('#branches-grid .glass-card h3');
            if (rows.length > 0) {
                branches = Array.from(rows).map((el, idx) => ({
                    id: idx + 1, // Warning: Dummy ID
                    name_ar: el.textContent.trim(),
                    name_en: el.textContent.trim()
                }));
                console.log('Scraped from DOM:', branches.length);
            }
        }
        ...
        if (branches.length > 0) {
            ...
            branches.forEach(branch => {
                // Determine ID
                const id = branch.id || branch.branch_id || branch.code;
            ...
            console.log('Branches rendered to select:', branchSelect.options.length - 1);
        } else {
            const option = document.createElement('option');
            option.textContent = "لا توجد فروع متاحة";
            option.disabled = true;
            branchSelect.appendChild(option);
            console.warn('No branches found from any source.');
        }

        // Restore selection
        if (currentVal) {
            branchSelect.value = currentVal;
            // Check if value was actually set (exists in options)
            if (branchSelect.value !== currentVal) {
                 // If ID exists but not in list, maybe add it as a placeholder?
                 // For now, just clear the dataset
            }
            delete branchSelect.dataset.selectedId;
        }
    };
...
(function(){
  ...
  function listMembers(){ try { return App.store.list(MEMBERS_COL) || []; } catch(e){ return []; } } 
  ...
  function renderMemberCostCenters(selectedArr){
    ...
    box.innerHTML = list.length ? '' : '<div style="color:#94a3b8;font-weight:900">لا توجد مراكز تكلفة محفوظة — تأكد من وجود بيانات الموظفين.</div>';
  ...
  function ensureMemberFromEmployee(emp){ 
    if(!emp) return null; 
    ...
    return upsertMember(m); 
  } 
  
  function syncMembersFromEmployees(){ 
    var emps = []; 
    try { emps = App.store.list('employees') || []; } catch(e){ emps = []; } 
    var created = 0; 
    emps.forEach(function(e){ 
      var before = listMembers().length; 
      try {
        var res = ensureMemberFromEmployee(e); 
        var after = listMembers().length; 
        if(res && after > before) created++; 
      } catch(err){}
    }); 
    if(typeof toast === 'function') toast('success','تم','تمت المزامنة. (جديد: ' + created + ')'); 
  ...
  window.addEventListener('load', function(){ 
    ...
    var syncBtn = document.getElementById('btn-members-sync'); if(syncBtn) syncBtn.onclick = syncMembersFromEmployees; 
    // Re-enable automatic sync on load for initial data population
...
function listComplaints() { return App.store.list(COMP_COL); }
function renderComplaints(filter) {
  ...
  list.forEach(function(c){
    ...
    tr.innerHTML = '<td class="p-4 text-slate-400 text-xs">#' + c.id + '</td>' +
      ...
      '<td class="p-4 text-slate-300">' + (c.type || '') + '</td>' +
...
window.renderViolationsTable = function(){
  ...
  function paidBadge(v){
    ...
    var txt = paid ? 'مدفوع' : (v.paid_status || 'غير مدفوع');
    var cls = paid ? 'bg-green-900 text-green-200' : 'bg-red-900 text-red-200';
    return '<span class="px-2 py-1 text-xs rounded '+cls+'">'+txt+'</span>';
  }

  tbody.innerHTML = pageRows.length ? '' : '<tr><td colspan="16" class="text-center text-slate-400 py-4">'+(App.t ? App.t('no.records') : 'لا توجد سجلات')+'</td></tr>';

  pageRows.forEach(function(v){
    var id = v.id || v.vio_no || v.violation_no || Date.now() + Math.random();
    var tr = document.createElement('tr');
    tr.innerHTML = ''+
      '<td><input type="checkbox" class="vio-select" data-id="'+id+'"></td>'+
      '<td>'+(v.branch||'-')+'</td>'+
      ...
      '<td>'+(v.amount||'-')+'</td>'+
      '<td>'+(v.date||'-')+'</td>'+
      '<td>'+(v.appeal_status||'-')+'</td>'+
      '<td>'+(v.appeal_date||'-')+'</td>'+
      '<td>'+(v.appeal_number||'-')+'</td>'+
      '<td>'+(v.finance_date||'-')+'</td>'+
      '<td>'+paidBadge(v)+'</td>'+
      '<td>'+
        '<button onclick="try{ editViolation(\''+id+'\'); }catch(e){}" class="text-blue-400 hover:text-blue-300 mr-2">تعديل</button>'+
        '<button onclick="try{ deleteViolationRecord(\''+id+'\'); }catch(e){}" class="text-red-400 hover:text-red-300">حذف</button>'+
      '</td>';
    tbody.appendChild(tr);
  });
